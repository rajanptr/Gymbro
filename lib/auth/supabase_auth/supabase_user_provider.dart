...
class GymbroSupabaseUser extends BaseAuthUser {
  GymbroSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gymbroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GymbroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
